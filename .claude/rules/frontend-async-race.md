...
## 主动跳过的补偿动作：记账补做，保护状态按完整作用域分桶

某个保护条件成立时主动跳过一次补偿动作（如补拉、对账、收尾切换）的，两条纪律：

- **跳过须记账，窗口关闭后补做**：被跳过的补偿不会自行再来，跳过方按触发它的 key 记账，保护窗口关闭的一侧负责补做；补做放 `finally` 一类同时覆盖成功与失败路径的位置，否则失败路径上窗口永不关闭、账永远挂着
- **保护状态按完整作用域分桶**：跨请求存活的保护状态（在途计数、跳过标志）按它语义上的完整作用域分桶——请求、项目、会话，缺一维就连坐：共享格子时并行的另一次操作会替这一次把保护摘掉，或别的作用域上迟迟不结束的操作一直压着本该发生的补偿。并发可叠加的用计数不用布尔

参考实现：`frontend/src/hooks/useAssistantSession.ts` 的 `deletingCurrentRef`（项目×会话计数分桶）与 `deferredRefreshRef`（跳过的列表补拉记账，删除收尾后补做）。
