...
- **测试形态**：集中矩阵（表自洽、规范解析全组合含三种 content_mode 的 reference_video → video_units、取证阶梯逐台阶、fail-loud）+ 每个含骨架分派的消费方一条遍历表键的穷尽性参数化断言——第五种骨架出现时，未处置的消费方逐个测试失败，而非复刻 ad 被静默跳过的路径。

## 明确不采用

- **`(content_mode, generation_mode)` 复合键注册表**——两轴确实共同决定规范骨架，但它们只在规范解析器里做紧凑映射；结构注册表仍以最终 kind 为键，避免为六种组合复制相同的 `id_field` / `chars_field` 事实。
